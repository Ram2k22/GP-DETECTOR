...
if not os.path.isdir(f'{os.getcwd()}/Results_txt'):
    os.makedirs(f'{os.getcwd()}/Results_txt')

filename = FLAGS.dataset + '_' + FLAGS.attack + '_' + str(FLAGS.num_data_in_class*10) + '_Detector.txt'
f = open(f'Results_txt/{filename}', 'w')
f.write(f'{filename}\n')
...
def load_img(dataset):
    location = os.getcwd() + '/adv_data/' + dataset
...
def get_last_hidden_output(adv_img, clean_img):
    ...
    for i in tqdm(range(min(len(clean_img),len(adv_img)))):
...
if __name__ == '__main__': 
    ...
    for iterate in iteration:
        ...
        m.save_model(f"saved_models/{FLAGS.dataset}_{FLAGS.attack}.gp")
        
        
        # loaded_model = GPy.models.GPClassification.load_model("saved_models/GPmodel.gp.zip")


        #=====================Single Image==================
        
        # path = os.getcwd() + "/check/CIFAR10/"
        ...
        #     img = kmodel.predict(preprocess_input2(image_buffer.copy()))
        #     label = m.predict(img)
        #     if label[0][0][0] >= 0.5 :
        #         # Plot the Adversarial image
        #         axes[i].imshow(image)
        #         axes[i].axis('off')
        #         axes[i].set_title(f"Adversarial:{np.argmax(img,axis = 1)}{files[i]}")
        #     else:
        #         # Plot the original image
        #         axes[i].imshow(image)
        #         axes[i].axis('off')
        #         axes[i].set_title(f"Original: {np.argmax(img,axis = 1)}{files[i]}")
