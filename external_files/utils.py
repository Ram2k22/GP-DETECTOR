...
def load_img(dataset, attack):
    location = os.getcwd() + '/adv_data/' + dataset + '/' + attack
...
def check_image_label(dataset, attack, file_idx=8):
    ...
    for i in tqdm(range(10), desc="%d th class checking label..."):
        a = np.load(os.getcwd() + f'/adv_data/{dataset}/{attack}/{str(i)}/{str(file_idx)}_origin.npy')
        b = np.load(os.getcwd() + f'/adv_data/{dataset}/{attack}/{str(i)}/{str(file_idx)}_adv.npy')
