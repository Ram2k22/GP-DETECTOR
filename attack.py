...
class AdvAttack():
    ...
    def _load_dataset_model(self):
        ...
        elif self.dataset == 'BT':
            self.num_classes = 4
            print("\n load data...")
            (self.x_train, self.y_train), (self.x_test, self.y_test) = braintumor().load_data()
            print("data loaded.")
    ...
    def _set_placeholder(self):
        ...
        if self.attack == 'BIM' or self.dataset =='BT' :
    ...
    def save_images(self, adv_x, save_dir):
        saved_class_idx = np.zeros(self.num_classes).astype('int32')
        ...
        for i in tqdm(range(len(self.x_test))):
            ...
            if self.attack == 'BIM' or self.dataset == 'BT':
                y_input = keras.utils.to_categorical(y_input, self.num_classes)
            ...
            # print('Labels : ',self.origin_preds[i][0],adv_pred[0],correct_label,self.origin_preds[i][0]==correct_label)
