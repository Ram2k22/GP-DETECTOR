...
flags = tf.app.flags
FLAGS = flags.FLAGS
flags.DEFINE_string('dataset', 'BT', 'Training dataset name.')
...
def evalute_save(adv_model):
    for i in tqdm(range(len(files))):
        ...
        if attack == 'BIM' or FLAGS.dataset == 'BT':
            y_input = keras.utils.to_categorical(y_input, n_classes)
...
# -----------------------   FGSM ATTACK   ------------------------------
    ...
    if FLAGS.dataset == 'CIFAR10' or FLAGS.dataset == 'BT':
    ...
    params = {'nb_candidate': n_classes, 'max_iter': 100, 'clip_min': 0., 'clip_max': 1., 'verbose':False}
